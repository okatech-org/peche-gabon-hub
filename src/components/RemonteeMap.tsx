...
export function RemonteeMap({ remontees, onRemonteeClick, height = "500px" }: RemonteeMapProps) {
  ...
  useEffect(() => {
    ...
    logger.info("🗺️ Initialisation de RemonteeMap...");
    ...
    try {
      ...
      map.current.on('load', () => {
        logger.info("✅ RemonteeMap chargée");
      ...
      map.current.on("error", (e) => {
        logger.warn("⚠️ Erreur RemonteeMap (ignorée si non critique)", e);
      });

    } catch (err) {
      logger.error("❌ Erreur initialisation RemonteeMap:", err);
    }
