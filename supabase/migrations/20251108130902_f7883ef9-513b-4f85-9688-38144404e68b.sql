...
CREATE TABLE public.workflow_templates (
  ...
  institution_emettrice_defaut TEXT[], -- Quelles institutions peuvent émettre
  institution_destinataire_defaut TEXT[], -- Quelles institutions peuvent recevoir
  
  -- Champs requis pour ce template
  champs_requis JSONB DEFAULT '[]', -- Liste des champs additionnels requis
  
  -- Configuration
  delai_traitement_jours INTEGER, -- Délai suggéré
  documents_requis TEXT[], -- Types de documents attendus
  
  -- Métadonnées
  actif BOOLEAN NOT NULL DEFAULT true,
  utilisation_count INTEGER NOT NULL DEFAULT 0,
  categorie TEXT NOT NULL DEFAULT 'general', -- 'licences', 'controles', 'infractions', 'statistiques', 'coordination'
...
RETURNS TRIGGER AS $$
BEGIN
  -- Si un workflow est créé à partir d'un template
